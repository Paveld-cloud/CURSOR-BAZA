...
async def send_welcome_sequence(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    card_html = (
        f"🌟 <b>Добро пожаловать, {first}!</b>\n\n"
        f"Вы в <b>умном каталоге запчастей</b> — быстро, удобно и красиво.\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"✨ <b>Ваши возможности:</b>\n\n"
        f"🔍 <b>Поиск за секунды</b>\n"
        f"• По названию, коду и парт-номеру\n"
        f"• Быстрый вход через категории\n"
        f"• История запросов под рукой\n\n"
        f"📦 <b>Работа с деталями</b>\n"
        f"• Карточки с фото и данными\n"
        f"• Списание в пару шагов\n"
        f"• Экспорт результатов в Excel\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🧪 <b>Попробуйте запрос:</b>\n"
        f"<code>PI8808DRG500</code>\n\n"
        f"💫 <i>Нажмите кнопку ниже и начнём!</i>"
    )
...
async def menu_search_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        "🔍 <b>Умный поиск</b>\n\n"
        "Введите <i>название</i>, <i>модель</i>, <i>код</i> или <i>парт-номер</i>.\n"
        "Можно писать с пробелами или без них.\n\n"
        "<b>Примеры:</b>\n"
        "• <code>PI 8808 DRG 500</code>\n"
        "• <code>фильтр топливный</code>\n"
        "• <code>W 75/3</code>"
    )
...
async def menu_issue_help_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    msg = (
        "📦 <b>Как красиво и быстро списать деталь</b>\n\n"
        "1) Найдите позицию через поиск или категории.\n"
        "2) В карточке нажмите <b>«📦 Взять деталь»</b>.\n"
        "3) Укажите количество и комментарий.\n"
        "4) Проверьте данные и подтвердите действие.\n\n"
        "✅ После подтверждения запись попадёт в журнал списаний."
    )
    await _safe_send_html_message(context.bot, q.message.chat_id, msg)
...
async def menu_contact_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _safe_send_html_message(
        ...
        q.message.chat_id,
        "📞 <b>Поддержка</b>\n\n"
        "Если нужна помощь с поиском, списанием или экспортом — напишите нам:\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        ...
        cmds_html = (
            ...
            "• /cancel — отмена текущего списания\n"
            "• /reload — перезагрузка данных и пользователей (админ)\n"
            "• /broadcast — рассылка сообщения пользователям (админ)\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "1) Напишите запрос: название / код / парт-номер.\n"
        "2) Выберите нужную карточку детали.\n"
        "3) Нажмите «📦 Взять деталь», введите количество и комментарий.\n"
        "4) Подтвердите списание.\n\n"
        "💡 Совет: используйте <b>Категории</b> и <b>Историю</b> для ещё более быстрого поиска."
    )
...
async def menu_help_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "⭐ <b>Избранное:</b>\n"
        "   • Добавляйте нужные позиции в один клик\n"
        "   • Быстро возвращайтесь к часто используемым деталям\n\n"
        "📦 <b>Списание:</b>\n"
